...
export class Group extends PureComponent {
  ...
  removeSelf() {
    ...
    const {
      renderConfirm,
      removeGroupConfirmOptions: confirmOptions,
  ...
  isEmpty(item) {
    return item.get('type') == 'group' || item.get('type') == 'rule_group'
      ? this.isEmptyGroup(item)
      : this.isEmptyRule(item);
  }
  ...
  renderItem(item) {
    ...
    const isRuleGroup =
      item.get('type') == 'group' && item.getIn(['properties', 'field']) != null;
  ...
  renderDrag() {
    const { config, isRoot, reordableNodesCnt, handleDraggerMouseDown } = this.props;
