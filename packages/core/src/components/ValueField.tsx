...
export default class ValueField extends PureComponent {
  ...
  onPropsChanged(nextProps) {
    ...
    const needUpdateItems =
      !this.items ||
      keysForItems.map((k) => nextProps[k] !== prevProps[k]).filter((ch) => ch).length >
        0;
  ...
  getMeta({ config, field, operator, value, placeholder: customPlaceholder, isFuncArg }) {
    ...
    const placeholder = isFieldSelected
      ? null
      : (isFuncArg && customPlaceholder) ||
        leftFieldWidgetFieldProps.valuePlaceholder ||
        fieldPlaceholder;
  ...
  filterFields(config, fields, leftFieldFullkey, operator, canCompareFieldWithField) {
    ...
    function _filter(list, path) {
      for (const rightFieldKey in list) {
        const { subfields } = list[rightFieldKey];
        const subpath = (path || []).concat(rightFieldKey);
        const rightFieldFullkey = subpath.join(fieldSeparator);
        const rightFieldConfig = getFieldConfig(rightFieldFullkey, config);
        if (!rightFieldConfig) {
          delete list[rightFieldKey];
        } else if (
          rightFieldConfig.type == '!struct' ||
          rightFieldConfig.type == '!group'
        ) {
  ...
  getFieldLabel(fieldOpts, fieldKey, config) {
    ...
    const fieldParts = Array.isArray(fieldKey)
      ? fieldKey
