...
export default (Builder, CanMoveFn = null) => {
  class SortableContainer extends Component {
      ...
      if (startDragging && startDragging.id) {
        ...
        if (dragging.itemInfo) {
          if (
            ...
            const { treeEl } = startDragging;
            const { treeElContainer } = startDragging;
            ...
            if (plhEl) {
              ...
              const { scrollTop } = treeElContainer;
    ...
    onDrag = (e, doHandleDrag = true) => {
      ...
      const startDragging = this.props.dragStart;
      const { paddingLeft } = dragging; // this.props.paddingLeft;
      const { treeElContainer } = startDragging;
      const { scrollTop } = treeElContainer;
      dragging.itemInfo = this.tree.items[dragging.id];
      if (!dragging.itemInfo) {
    ...
    onDragEnd = () => {
      const { treeEl } = this.props.dragStart;
    ...
    handleDrag(dragInfo, e, canMoveFn) {
      ...
      const { itemInfo } = dragInfo;
      const { paddingLeft } = dragInfo;
      ...
      if (dragEl && plhEl) {
        ...
        if (!hovCNodeEl) {
          ...
          if (hovEl) {
            ...
            let trgEl = null;
            let trgII = null;
            let altII = null; // for canMoveBeforeAfterGroup
            ...
            if (trgRect) {
              ...
              let availMoves = [];
              const altMoves = []; // alternatively can move after/before group, if can't move into it
              if (isSamePos) {
                // do nothing
              } else {
                if (isGroup) {
              ...
              let filteredMoves = availMoves.filter((am) =>
                this.canMove(itemInfo, am[1], am[0], am[3], canMoveFn)
              );
              if (
    ...
    canMove(fromII, toII, placement, toParentII, canMoveFn) {
      ...
      const { canRegroup } = this.props.config.settings;
