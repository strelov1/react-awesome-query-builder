...
export default (Widget: ReactElement<any>) => {
  return class WidgetContainer extends PureComponent<WidgetContainerProps> {
    ...
    getMeta({
      ...
      const { isSpecialRange } = operatorDefinition;
      const isSpecialRangeForSrcField =
        isSpecialRange && (_valueSrcs.get(0) == 'field' || _valueSrcs.get(1) == 'field');
...
const WidgetFactory = ({
  ...
  let value = isSpecialRange ? [immValue.get(0), immValue.get(1)] : immValue.get(delta);
  const valueError =
    (immValueError &&
      (isSpecialRange
