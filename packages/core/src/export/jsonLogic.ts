...
const jsonLogicFormatValue = (
  ...
  if (valueSrc == 'field') {
    ...
    for (const argKey in funcConfig.args) {
      ...
      const formattedArgVal = jsonLogicFormatValue(
        meta,
        config,
        argValue,
        argValueSrc,
...
const jsonLogicFormatItem = (item, config, meta, isRoot, parentField = null) => {
  ...
  if ((type === 'group' || type === 'rule_group') && children && children.size) {
    ...
    const list = children
      .map((currentChild) =>
        jsonLogicFormatItem(currentChild, config, meta, false, groupField)
      )
    ...
    if (not && !isRuleGroup) {
      if (
        ...
        ['all', 'some'].includes(Object.keys(resultQuery)[0])
      ) {
  ...
  if (type === 'rule') {
    ...
    const valueSrcs = [];
    const valueTypes = [];
    const _usedFields = meta.usedFields;
