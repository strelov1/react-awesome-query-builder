...
const mongoFormatValue = (
  ...
  if (valueSrc == 'field') {
    ...
    if (rightField) {
      ...
      const rightFieldName = Array.isArray(rightField)
    ...
    const { mongoArgsAsObject } = funcConfig;
    const formattedArgs = {};
    let argsCnt = 0;
    let lastArg;
    for (const argKey in funcConfig.args) {
      ...
      const [formattedArgVal, _argUseExpr] = mongoFormatValue(
        meta,
        config,
        argValue,
        argValueSrc,
    ...
    if (typeof funcConfig.mongoFormatFunc === 'function') {
      ...
      ret = fn(...args);
    } else if (mongoArgsAsObject) ret = { [funcName]: formattedArgs };
    else if (argsCnt == 1 && lastArg !== undefined) ret = { [funcName]: lastArg };
    else ret = { [funcName]: Object.values(formattedArgs) };
  } else if (typeof fieldWidgetDefinition.mongoFormatValue === 'function') {
    const fn = fieldWidgetDefinition.mongoFormatValue;
    const args = [
      currentValue,
      pick(fieldDefinition, ['fieldSettings', 'listValues']),
      // useful options: valueFormat for date/time
      omit(fieldWidgetDefinition, [
        'formatValue',
        'mongoFormatValue',
        'sqlFormatValue',
        'jsonLogic',
...
const mongodbFormatItem = (parents, item, config, meta, _not = false) => {
  ...
  if ((type === 'group' || type === 'rule_group') && children && children.size) {
    ...
    const list = children
      .map((currentChild) =>
        mongodbFormatItem([...parents, item], currentChild, config, meta, not)
      )
      .filter((currentChild) => typeof currentChild !== 'undefined');
    if (!list.size) return undefined;

    const field = type === 'rule_group' ? properties.get('field') : null;
    let conjunction = properties.get('conjunction');
    if (!conjunction) conjunction = defaultConjunction(config);
    let conjunctionDefinition = config.conjunctions[conjunction];
    const { reversedConj } = conjunctionDefinition;
    if (not && reversedConj) {
      conjunction = reversedConj;
      conjunctionDefinition = config.conjunctions[conjunction];
    }
    const { mongoConj } = conjunctionDefinition;

    let resultQuery;
    if (list.size == 1) resultQuery = list.first();
    else {
  ...
  if (type === 'rule') {
    ...
    let { reversedOp } = operatorDefinition;
    ...
    value = value.map((currentValue, ind) => {
      const valueSrc = properties.get('valueSrc')
        ? properties.get('valueSrc').get(ind)
        : null;
      const valueType = properties.get('valueType')
        ? properties.get('valueType').get(ind)
        : null;
      ...
      const [fv, _useExpr] = mongoFormatValue(
        meta,
        config,
        currentValue,
        valueSrc,
    ...
    const formattedValue =
      cardinality > 1 ? value.toArray() : cardinality == 1 ? value.first() : null;
