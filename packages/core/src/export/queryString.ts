...
const formatValue = (
  ...
  if (valueSrc == 'field') {
    ...
    if (typeof funcConfig.formatFunc === 'function') {
      ...
      ret = `${funcName}(${Object.entries(formattedArgsWithNames)
        .map(([k, v]) => (isForDisplay ? `${k}: ${v}` : `${v}`))
        .join(', ')})`;
    }
  } else if (typeof fieldWidgetDefinition.formatValue === 'function') {
...
export const queryString = (item, config, isForDisplay = false) => {
  ...
  if (type === 'rule') {
    ...
    const { reversedOp } = operatorDefinition;
    ...
    const valueSrcs = [];
    const valueTypes = [];
    const value = properties.get('value').map((currentValue, ind) => {
    ...
    if (!fn && cardinality == 1) {
      const _operator = operatorDefinition.labelForFormat || operator;
      fn = (
        field,
        op,
        values,
    ...
    const fieldFullLabel = fieldPartsLabels
      ? fieldPartsLabels.join(config.settings.fieldSeparatorDisplay)
      : null;
