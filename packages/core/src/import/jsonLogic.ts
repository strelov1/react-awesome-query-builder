...
const buildConv = (config) => {
  ...
  for (const opKey in config.operators) {
    ...
    if (typeof opConfig.jsonLogic === 'string') {
      // example: "</2", "#in/1"
      const opk = `${
        (opConfig._jsonLogicIsRevArgs ? '#' : '') + opConfig.jsonLogic
...
const convertFromLogic = (
  ...
  if (isRev) {
    ...
    ret =
      convertConj(op, vals, conv, config, not, meta, parentField) ||
...
const convertConj = (op, vals, conv, config, not, meta, parentField = null) => {
  ...
  if (conjKey) {
    ...
    const children = vals
      .map((v) =>
        convertFromLogic(v, conv, config, 'rule', meta, false, null, null, parentField)
      )
    ...
    const complexFieldsConfigs = complexFieldsParents
      ...
      .to_object();
    const complexFieldsInRuleGroup = complexFieldsParents.filter(
    ...
    const needSplit = !(
      usedTopRuleGroups.length == 1 &&
      complexFieldsInRuleGroup.length == Object.keys(children).length
    );
    const groupToId = {};
    Object.entries(children).map(([k, v]) => {
      if (v.type == 'group' || v.type == 'rule_group') {
        ...
        const groupFields = usedRuleGroups.filter(
          (f) => v.properties.field.indexOf(f) == 0
        );
        const groupField =
          groupFields.length > 0
            ? groupFields.sort((a, b) => b.length - a.length)[0]
            : null;
        if (!groupField) {
...
const topLevelFieldsFilter = (fields) => {
  const arr = [...fields].sort((a, b) => a.length - b.length);
  ...
  if (['some', 'all', 'none'].includes(op) && arity == 2) {
    if (vals[0].var !== undefined && Object.keys(vals[1]).length == 1) {
      ...
      if (!(newOp == 'in' && op == 'all')) {
        ...
        const groupField = (parentField ? [parentField, field] : [field]).join(
          fieldSeparator
        );
