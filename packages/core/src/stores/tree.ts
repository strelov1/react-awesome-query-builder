...
const removeRule = (state, path, config) => {
  ...
  const canLeaveEmpty =
    isEmptyGroup &&
    (isParentRuleGroup ? true : config.settings.canLeaveEmptyGroup && !isEmptyRoot);
  if (isEmptyGroup) {
    if (isParentRuleGroup) {
      ...
      state = addItem(
        ...
        'rule',
        uuid(),
        defaultRuleProperties(config, parentField),
        config
      );
    } else if (!canLeaveEmpty) {
      state = fixEmptyGroupsInTree(state);
...
const addItem = (state, path, type, id, properties, config) => {
  ...
  const canAddNewRule = !(
    type == 'rule' &&
    maxNumberOfRules &&
    rulesNumber + 1 > maxNumberOfRules
  );
...
const moveItem = (state, fromPath, toPath, placement, config) => {
  ...
  const targetPath =
    placement == constants.PLACEMENT_APPEND || placement == constants.PLACEMENT_PREPEND
      ? toPath
      : toPath.pop();
  const target =
    placement == constants.PLACEMENT_APPEND || placement == constants.PLACEMENT_PREPEND
  ...
  let newSourceChildren = sourceChildren;
  ...
  if (placement == constants.PLACEMENT_BEFORE || placement == constants.PLACEMENT_AFTER) {
    newTargetChildren = Immutable.OrderedMap().withMutations((r) => {
      for (const [itemId, item] of newTargetChildren.entries()) {
...
const setField = (state, path, newField, config) => {
  ...
  if (isRuleGroup) {
    ...
    const groupProperties = defaultGroupProperties(config).merge({
      field: newField,
    });
...
const setValue = (state, path, delta, value, valueType, config, __isInternal) => {
  ...
  if (showErrorMessage) {
    ...
    if (
      ...
      valueSrcs.filter((vs) => vs == 'value' || vs == null).length == operatorCardinality
    ) {
      const values = Array.from({ length: operatorCardinality }, (_, i) =>
        i == delta
          ? value
          : state.getIn(expandTreePath(path, 'properties', 'value', `${i}`)) || null
      );
      const jsValues =
        fieldWidgetDefinition && fieldWidgetDefinition.toJS
          ? values.map((v) => fieldWidgetDefinition.toJS(v, fieldWidgetDefinition))
          : values;
...
export default (config) => {
  ...
  const emptyState = { tree: emptyTree, ...emptyDrag };
  const unset = { __isInternalValueChange: undefined };

  return (state = emptyState, action) => {
    switch (action.type) {
      case constants.SET_TREE:
        return { ...state, ...unset, tree: action.tree };
