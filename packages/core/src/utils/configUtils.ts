      ...
      if (!isFuncArg) {
        ...
        if (fieldWidgetConfig.operators) {
          ...
          operators = operators.concat(
            fieldWidgetConfig.operators.filter((o) => !excludeOperators.includes(o))
          );
...
export const getFirstField = (config, parentRuleGroupPath = null) => {
  ...
  const parentField = parentRuleGroupPath
    ? getFieldRawConfig(parentRuleGroupPath, config)
    : config;
...
export const getFieldWidgetConfig = (
  ...
  const valueFieldSettings =
    ((valueSrc == 'value' || !valueSrc) && fieldConfig && fieldConfig.fieldSettings) ||
...
function _getWidgetsAndSrcsForFieldOp(config, field, operator = null, valueSrc = null) {
  ...
  widgets.sort((w1, w2) => {
    const w1Main = fieldConfig.preferWidgets
      ? fieldConfig.preferWidgets.indexOf(w1) != -1
      : w1 == fieldConfig.mainWidget;
    const _w2Main = fieldConfig.preferWidgets
      ? fieldConfig.preferWidgets.indexOf(w2) != -1
      : w2 == fieldConfig.mainWidget;
    if (w1 != w2) {
      return w1Main ? -1 : +1;
    }
    return 0;
