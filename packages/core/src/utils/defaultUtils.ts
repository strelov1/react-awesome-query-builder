...
import { getNewValueForFieldOp } from './validation';
...
export const defaultOperatorOptions = (config, operator, field) => {
  const operatorConfig = operator ? getOperatorConfig(config, operator, field) : null;
  if (!operatorConfig) return null; // new Immutable.Map();
  return operatorConfig.options
    ? new Immutable.Map((operatorConfig.options && operatorConfig.options.defaults) || {})
    : null;
};
...
export const defaultRuleProperties = (config, parentRuleGroupPath = null) => {
  ...
  if (field && operator) {
    const { newValue, newValueSrc, newValueType, newValueError } = getNewValueForFieldOp(
      config,
      config,
      current,
      field,
...
export const defaultGroupProperties = (config) =>
  new Immutable.Map({
