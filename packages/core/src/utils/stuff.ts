...
export const shallowEqual = (a, b, deep = false) => {
  ...
  if (Array.isArray(a)) return shallowEqualArrays(a, b, deep);
  if (Map.isMap(a)) return a.equals(b);
  if (typeof a === 'object') return shallowEqualObjects(a, b, deep);
  return a === b;
};
...
export const onPropsChanged = (obj) => {
  if (canUseUnsafe) {
...
export const getItemInListValues = (listValues, value) => {
  if (Array.isArray(listValues)) {
    ...
    return (
      values.find((v) => v.value === value) || values.find((v) => `${v.value}` === value)
    );
  }
