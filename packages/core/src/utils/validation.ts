...
function validateRule(item, path, itemId, meta, c) {
  ...
  const _wasValid =
    field && operator && value && !value.find((v, ind) => v === undefined);
  ...
  const _operatorCardinality = operator
  ...
  const { newValue, newValueSrc, newValueError } = getNewValueForFieldOp(
    config,
    oldConfig,
    properties,
    field,
  ...
  if (value != null) {
    ...
    if (!validError) {
      ...
      const rightFieldDefinition =
        valueSrc == 'field' ? getFieldConfig(value, config) : null;
...
export const getNewValueForFieldOp = function (
  ...
  const valueFixes = {};
  const valueErrors = Array.from({ length: operatorCardinality }, () => null);
  if (canReuseValue) {
    for (let i = 0; i < operatorCardinality; i++) {
  ...
  let newValue = null;
  let newValueSrc = null;
  let newValueType = null;
  let newValueError = null;
